...
class Chatbot:
    def __init__(self, master):
        self.master = master
        master.title("Netflix Assistant")
        ...
        self.text_area.insert(ctk.END, "Olá! Eu sou seu assistente Netflix. Você pode fazer perguntas como:\n\n" +
                            "1. Buscar por título: 'Mostre informações sobre [título]'\n" +
                            "2. Buscar por diretor: 'Quais filmes são dirigidos por [diretor]?'\n" +
    ...
    def load_data(self):
        try:
            file_path = 'netflix_movies (1).csv'
            if not os.path.exists(file_path):
                print("Baixando dataset...")
                url = 'https://drive.google.com/uc?id=11QNHRhwlRu8uJGoCdbQA6PwhiRu6FQNm'
    ...
    def search_by_title(self, title):
        matches = self.data[self.data['title'].apply(self.normalize_string).str.contains(title, na=False)]
        if matches.empty:
        ...
        return f"Título: {result['title']}\n" + \
               ...
               f"Ano: {result['release_year']}\n" + \
               f"Duração: {result['duration']}\n" + \
               f"Gêneros: {result['listed_in']}\n" + \
    ...
    def search_by_year(self, year):
        matches = self.data[self.data['release_year'] == year]
        if matches.empty:
    ...
    def search_by_genre(self, genre):
        matches = self.data[self.data['listed_in'].apply(self.normalize_string).str.contains(genre, na=False)]
        if matches.empty:
    ...
    def get_statistics(self):
        ...
        movies = len(self.data[self.data['type'] == 'Movie'])
        tv_shows = len(self.data[self.data['type'] == 'TV Show'])
        
        top_directors = self.data['director'].value_counts().head(5)
        top_genres = self.data['listed_in'].str.split(', ', expand=True).stack().value_counts().head(5)
        
        stats = f"Estatísticas gerais:\n\n" \
                f"Total de títulos: {total_titles}\n" \
                f"Filmes: {movies}\n" \
                f"Séries de TV: {tv_shows}\n\n" \
                f"Top 5 diretores:\n"
